...
export function WatchedPage() {
  ...
  const removeMutation = useMutation({
    mutationFn: watchedApi.remove,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["watched"] }); toast.success("Removed from watched"); },
    ...
    <div className="container py-8">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Movies you've already seen • {movies?.length || 0} movies</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState icon={<Eye className="h-16 w-16" />} title="No watched movies yet" description="Mark movies as watched to track them here" actionLabel="Browse Movies" actionHref="/browse" />
      )}
    </div>
  );
}
